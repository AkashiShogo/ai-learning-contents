...
会話モードに入らず、1つの質問だけ投げて答えをもらうこともできる。

```bash
gemini -p "gitのcommitとpushの違いを1行で説明して"
```

`-p`（prompt）オプションで質問を渡すと、回答を表示して終了する。シェルスクリプトや他のコマンドとの組み合わせ（パイプ）で活躍する形で、第5章以降でよく使う。

> オプションやファイルの渡し方など、より高度な使い方は第5章で学ぶ。

### ツールの操作許可について

Gemini CLI にはファイルの読み書きやコマンド実行をAIが行う機能もある。その際は実行前に許可を求められる。**第2章2-7で学んだとおり、許可プロンプトは雑に承認せず、何をしようとしているかを毎回確認する。** 許可の管理は第5章で実際に扱う。
...
出てきたエラーメッセージをそのままコピーして、Gemini に貼り付けて聞く。
...
2. `.gitignore` に書いたファイルはどうなる？`.env` を書く理由は？（→ 3-6）
3. SSH接続を設定すると何が楽になる？（→ 3-4）
4. `gemini` と `gemini -p "..."` の違いは？それぞれどんな場面で使う？（→ 3-8）
5. Web UI と CLI の使い分けの判断基準を自分の言葉で説明できる？（→ 3-9）
